...
namespace Elendow.SpritedowAnimator
{
    ...
    public class EditorPreviewSpriteAnimation : Editor
    {
        ...
        private int eventListSelectedIndex = -1;
        ...
        private List<SpriteAnimationEvent> events;
        ...
        private void InitializeReorderableList()
        {
            ...
            if (events == null)
                events = new List<SpriteAnimationEvent>();
            events.Clear();
            for (int i = 0; i < animation.Events.Count; i++)
            {
                events.Add(new SpriteAnimationEvent()
                {
                    eventFrame = animation.Events[i].eventFrame,
                    eventName = animation.Events[i].eventName,
                    messageOptions = animation.Events[i].messageOptions,
                });
            }
            ...
            if (eventList != null)
            {
                eventList.drawHeaderCallback -= DrawEventListHeader;
                eventList.drawElementCallback -= DrawEventListElement;
                eventList.onAddCallback -= AddEventListItem;
                eventList.onRemoveCallback -= RemoveEventListItem;
                eventList.onSelectCallback -= SelectEventListItem;
                eventList.onReorderCallback -= ReorderEventListItem;
            }

            eventList = new ReorderableList(events, typeof(SpriteAnimationEvent));
            eventList.drawHeaderCallback += DrawEventListHeader;
            eventList.drawElementCallback += DrawEventListElement;
            eventList.onAddCallback += AddEventListItem;
            eventList.onRemoveCallback += RemoveEventListItem;
        ...
        private void DrawEventListHeader(Rect r)
        {
            GUI.Label(r, "Event List");
        }

        private void DrawEventListElement(Rect r, int i, bool active, bool focused)
        {
            if (speedScale == null)
                speedScale = EditorGUIUtility.IconContent("SpeedScale");
            ...
            if (i < animation.Events.Count)
            {
                ...
                string eventName = (animation.Events[i] != null) ? animation.Events[i].eventName : "Unnamed";
                EditorGUIUtility.labelWidth = r.width - 250;
                animation.Events[i].eventName = EditorGUI.TextField(new Rect(r.x + 10, r.y + 1, r.width - 85, r.height - 4), $"Event {i}", animation.Events[i].eventName);

                EditorGUIUtility.labelWidth = 20;
                animation.Events[i].eventFrame = EditorGUI.IntField(new Rect(r.x + r.width - 50, r.y + 1, 50, r.height - 4), speedScale, animation.Events[i].eventFrame);
        ...
        private void AddEventListItem(ReorderableList list)
        {
            Undo.RecordObject(animation, "Add Event");
            AddEvent();
            EditorUtility.SetDirty(animation);
        }
        ...
        {
            eventList.list.Add(new SpriteAnimationEvent() { eventName = "NewEvent", eventFrame = animation.AnimationDuration });
            animation.Events.Add(null);
        }
        ...
        private void ReorderEventListItem(ReorderableList list)
        {
            Undo.RecordObject(animation, "Reorder Frames");

            SpriteAnimationEvent sae = animation.Events[eventListSelectedIndex];
            animation.Events.RemoveAt(eventListSelectedIndex);
            animation.Events.Insert(list.index, sae);

            EditorUtility.SetDirty(animation);
        }

        private void SelectEventListItem(ReorderableList list)
        {
            currentEvent = list.index;
            forceRepaint = true;
            eventListSelectedIndex = list.index;
        }
