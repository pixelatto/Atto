...
public partial class PowerSpriteAnimator
{
	...
	static readonly System.Type TYPE_SPRITEANIMNODES = typeof(PowerSpriteAnimNodes);
	...
	{
		public KeyframeReadInData( System.Action<PowerAnimFrame, int, float> functionSetNodeAtFrame, Keyframe keyframe )
		{
		...
		public System.Action<PowerAnimFrame, int, float> m_functionSetNodeAtFrame = null;
	...
	[SerializeField] Vector2[] m_nodePositions = new Vector2[PowerSpriteAnimNodes.NUM_NODES]; // positions of the nodes
	[SerializeField] float[] m_nodeAngles = new float[PowerSpriteAnimNodes.NUM_NODES]; // angles of the nodes
	...
	void LayoutAnimationNodesPanel( Rect rect )
	{
		...
		PowerAnimFrame currentFrame = GetCurrentFrame();
		...
		for ( int i = PowerSpriteAnimNodes.NUM_NODES-1; i >= 0; --i ) 
		{
			if ( i != m_selectedNode  && Utils.BitMask.IsSet(m_visibleNodes,i) )
	...
	void LayoutNode( Rect rect, int nodeId, PowerAnimFrame currentFrame )
	{
	...
	{		
		...
		if ( m_nodePositions == null || m_nodePositions.Length == 0 )
			m_nodePositions = new Vector2[PowerSpriteAnimNodes.NUM_NODES];
		if ( m_nodeAngles == null || m_nodeAngles.Length == 0 )
			m_nodeAngles = new float[PowerSpriteAnimNodes.NUM_NODES];
	...
	void MoveNode( int id, Vector2 newOffset, bool snapToPixel = true )
	{
		...
		PowerAnimFrame frame = GetCurrentFrame();
		if ( frame == null )
			return;
	...
	void SetNodeAtFrame(PowerAnimFrame frame, int nodeId, Vector2 offset )
	{
	...
	void SetNodeOffsetAtFrame(PowerAnimFrame frame, int nodeId, Vector2 offset )
	{
		Node node = FindOrCreateNodeAtFrame(frame, nodeId);
		if ( offset == Vector2.zero )
			offset = new Vector2(0.0001f,0.0001f); // Zero nodes are stripped, so when setting to zero, actually set to a real low value
	...
	void SetNodeAngleAtFrame(PowerAnimFrame frame, int nodeId, float angle )
	{
		if ( frame != null )
	...
	void SetNodeXAtFrame(PowerAnimFrame frame, int nodeId, float value )
	{
		Node node = FindOrCreateNodeAtFrame(frame, nodeId);
	...
	void SetNodeYAtFrame(PowerAnimFrame frame, int nodeId, float value )
	{
		Node node = FindOrCreateNodeAtFrame(frame, nodeId);
		// if (value == 0 && onLoad == false ) - This function is only called on load
		// 	value = 0.0001f; // Zero nodes are stripped, so when setting to zero, actually set to a real low value
	...
	Node FindOrCreateNodeAtFrame(PowerAnimFrame frame, int nodeId)
	{
		if ( frame == null )
			return null;
	...
	void ReadNodesFromClip()
	{
		...
		for ( int nodeId = 0; nodeId < PowerSpriteAnimNodes.NUM_NODES; ++nodeId )
		{
	...
	*/
	...
	/*
	void ReadNodesToKeyFrameReadInData( EditorCurveBinding[] bindings, System.Action<PowerAnimFrame, int, float> functionSetNodeAtFrame, List<KeyframeReadInData> keyframes, int nodeId )
	{
	...
	*/
	...
	void ApplyNodeChanges()
	{
		...
		AnimationCurve[] curvesX = new AnimationCurve[PowerSpriteAnimNodes.NUM_NODES];
		AnimationCurve[] curvesY = new AnimationCurve[PowerSpriteAnimNodes.NUM_NODES];
		AnimationCurve[] curvesAngle = new AnimationCurve[PowerSpriteAnimNodes.NUM_NODES];

		// Cached node values to check 
		float[] prevValueX = new float[PowerSpriteAnimNodes.NUM_NODES];
		float[] prevValueY = new float[PowerSpriteAnimNodes.NUM_NODES];
		float[] prevValueAngle = new float[PowerSpriteAnimNodes.NUM_NODES];
	...
	*/
	...
	void CreateStaticBindings()
	{
		...
		{
			NODE_BINDINGS_X = new EditorCurveBinding[PowerSpriteAnimNodes.NUM_NODES];
			NODE_BINDINGS_Y = new EditorCurveBinding[PowerSpriteAnimNodes.NUM_NODES];
			NODE_BINDINGS_ANGLE = new EditorCurveBinding[PowerSpriteAnimNodes.NUM_NODES];
	...
	void UpdateCurrentFrameNodePositions()
	{
		...
		{	
			...
			{
				Node node = null;
				PowerAnimFrame frame = m_frames[frameId];
				if ( frame.m_nodes != null )
					node = frame.m_nodes.Find(item=>item.m_nodeId == nodeId);
	...
	void InitNodeValues( Node node, PowerAnimFrame frame )
	{
		...
		for ( int frameId = m_frames.FindIndex(item=>item == frame)-1; frameId >= 0; --frameId )
		{
			...
			PowerAnimFrame prevFrame = m_frames[frameId];
