...
public partial class PowerSpriteAnimator
{
	...
	[SerializeField] List<PowerAnimEvent> m_acListAll = new List<PowerAnimEvent>();
	List<PowerAnimEvent> m_acList = new List<PowerAnimEvent>();
	...
	void LayoutTimeline( Rect rect )
	{
        ...
        m_timelineScale = Mathf.Clamp( m_timelineScale, 1, 10000 );
		...
		if ( rect.Contains( e.mousePosition ) )
		{
			...
			if ( e.type == EventType.ScrollWheel )
			{
                ...
                int oldCursorTime = GuiPosToAnimTime(rect, e.mousePosition.x);

				m_timelineScale -= e.delta.y * scale * 0.05f;
				m_timelineScale = Mathf.Clamp(m_timelineScale,1.0f,10000.0f);
		...
		if ( doAutoComplete )
		{
			PowerAnimEvent autoCompleteEvent = DoAutoComplete(m_selectedEvents[0].m_functionName);
			if ( autoCompleteEvent != null )
			{	
				PowerAnimEvent selectedEvent = m_selectedEvents[0];
	...
	void LayoutScrubber(Rect rect )
	{
        ...
        //
		float minUnitSecond = 1.0f/m_clip.FPS;		
                ...
                if ( i % lodIntervalList[idx] == 0 ) 
                {
                    ...
                    if ( lodWidthList[idx] >= SCRUBBER_INTERVAL_TO_SHOW_LABEL ) 
                    {
						GUI.Label ( new Rect( x + 4.0f, -2, 50, 15 ), 
							ToTimelineLabelString(i*curUnitSecond, m_clip.FPS), EditorStyles.miniLabel );
                    }
	...
	void LayoutEvents(Rect rect)
	{
		...
		{
			// Move timeline
			m_animTime = (int)(GuiPosToAnimTime(rect, e.mousePosition.x));	
		}
		if ( e.type == EventType.MouseDown && e.button == 0 && e.clickCount == 2 && rect.Contains(e.mousePosition) )
		{
			...
			// New event
			InsertEvent((int)(GuiPosToAnimTime(rect, e.mousePosition.x)), true);
			e.Use();			
		}
		...
		{
			PowerAnimEvent animEvent = m_events[i];
			AnimEventLayoutData eventData = new AnimEventLayoutData();
			eventTimelineData[i] = eventData;

			eventData.start = AnimTimeToGuiPos(rect, (int)( animEvent.m_time ) );
			eventData.text = animEvent.m_functionName;
	...
	void LayoutEvent(Rect rect, PowerAnimEvent animEvent, AnimEventLayoutData layoutData, bool draw )
	{	
	...
	void LayoutEventVisuals(PowerAnimEvent animEvent, bool selected, Rect eventRect, string labelText, Rect labelRect )
	{	
	...
	void LayoutEventGuiEvents(Rect rect, PowerAnimEvent animEvent, bool selected, Rect eventRect, string labelText, Rect labelRect )
	{	
	...
	void LayoutFrame( Rect rect, int frameId, int startTime, float endTime, Sprite sprite )
	{
		...
		if ( startOffset > rect.xMax || endOffset < rect.xMin )
			return;
		PowerAnimFrame animFrame = m_frames[frameId];
			...
			{	
				...
				{					
					...
					{						
						for ( int i = 0; i < m_selectedFrames.Count; ++i )
						{
							m_selectedFrames[i].m_length = Mathf.Max(1, m_selectedFrames[i].m_length + (1 * direction));
						}	
						RecalcFrameTimes();
					}
				}
				...
				{
					int newFrameLength = GuiPosToAnimTime(new Rect(0,0,position.width,position.height), e.mousePosition.x) - startTime;
					newFrameLength = Mathf.Max(newFrameLength, 1);
	...
	void LayoutBottomBarEventData(Rect rect)
	{
		...
		// Function Name
		PowerAnimEvent animEvent = m_selectedEvents[0];
		...
		GUI.Label( new Rect(xOffset,0, width,rect.height), $"Frame: {Mathf.RoundToInt(animEvent.m_time)}", EditorStyles.miniLabel );
	...
	{
		PowerAnimFrame frame = m_selectedFrames[0];
    ...
    List<int> CreateIntervalSizeList(out int intervalId)
    {
        ...
        int tmpSampleRate = (int)m_clip.FPS;
        while ( true ) {
            int div = 0;
            if ( tmpSampleRate == 30 ) {
                div = 3;
    ...
    int GuiPosToAnimTime(Rect rect, float mousePosX)
    {
		float pos = mousePosX - rect.xMin;
		return Mathf.RoundToInt((pos-m_timelineOffset) / m_timelineScale );
    }

    float AnimTimeToGuiPos(Rect rect, int time)
    {
	...
	PowerAnimEvent DoAutoComplete(string text)
	{
		if ( text.Length == 0 )
			return null;
